...
当前产物为 `apps/windows/ai/config/Fowan.Ai.Config.Windows.csproj`，构建输出为 `build/windows/win-x64/app/Tools/AI/Config/Fowan.Ai.Config.Windows.exe`，发布目录为 `Tools/AI/Config`。
...
- 测试连接只能由用户主动触发。
- 测试密钥时必须已有归属于该密钥且已启用的模型；否则提示用户先添加模型，不发送云端请求。
