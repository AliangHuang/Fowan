...
当前产物为 `apps/windows/ai/chat/Fowan.Ai.Chat.Windows.csproj`，构建输出为 `build/windows/win-x64/app/Tools/AI/Chat/Fowan.Ai.Chat.Windows.exe`，发布目录为 `Tools/AI/Chat`。
...
- 概念图中的附件、图片和 `@` 图标仅作为不可点击、不可聚焦的视觉装饰；它们不表示首版可用能力，也不得触发附件、多模态或工具调用流程。
...
- Debug 构建可通过 `--visual-fixture` 启动固定的 AI 对话视觉快照，用于概念图截图比对。
- 夹具不得连接 Core、读取或写入用户数据、持久化示例会话，Release 构建不得提供该入口。
