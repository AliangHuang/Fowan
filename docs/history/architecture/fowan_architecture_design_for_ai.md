# Fowan 开源客户端架构设计（历史草案）

> 状态：已归档。本文记录 AI/Core 接入前的架构设想，不再作为当前实现的事实源。
>
> 当前事实源：[`repository_boundaries.md`](../../repository_boundaries.md)、[`protocol/ai/v0.1`](../../../protocol/ai/v0.1/README.md) 与对应 ADR。

## 1. 架构摘要

Fowan 使用“开源原生客户端 + 独立闭源核心”的长期架构，但本文写作时尚未接入闭源核心。
...
负责窗口、布局、主题、输入、可访问性、平台权限入口和用户可见错误。UI 可以持有临时视图状态，但不应复制私有核心的 AI、加密或商业策略。
...
- 普通本地数据读写仍属于公开工具实现。
- 用户信息加解密、密钥生成、保存、轮换和恢复策略进入 FowanCore。
...
- 新增跨仓库能力前先更新 `repository_boundaries.md`。
- 不允许公开客户端直接依赖 FowanCore 的内部 Rust crate、存储结构或私有模型。
