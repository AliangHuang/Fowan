...
已完成任务在主窗口详情中显示可编辑的完成日期和完成时间；两个编辑器均为 236 DIP 宽，分钟列和右缘操作区必须完整可见。修改后先显示“应用修改”“撤销修改”，仅应用会写入，切换任务或视图自动撤销草稿。主窗口和便签任务卡在鼠标悬浮时使用低对比、与主题匹配的 8 DIP 圆角高亮，不能使用直角白色焦点框或影响文字可读性的亮色背景。

### 11.7 主窗口虚拟标题栏

主窗口顶部全宽虚拟标题栏包含侧栏品牌和无 UI 空白区，用户可在该区域拖动窗口。系统窗口控件与所有可交互 UI 必须保留客户端命中：筛选、便签切换、侧栏导航/清单菜单、输入框、任务卡和任务操作均不可因标题栏而失去点击能力。
