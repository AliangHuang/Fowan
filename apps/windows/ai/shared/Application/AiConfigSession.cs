...
public interface IAiConfigCommands
{
    ...
    Task CreateCredentialAsync(string channelId, string label, string? endpoint, string secret,
        IReadOnlyList<string>? initialModelIds, bool? thinkingEnabled, CancellationToken cancellationToken = default);
    Task DeleteCredentialAsync(string id, CancellationToken cancellationToken = default);
    Task UpsertModelAsync(string? id, string credentialId, string modelId, string displayName, string source, int contextWindowTokens, int maxOutputTokens,
        bool? thinkingEnabled, CancellationToken cancellationToken = default);
...
public sealed class AiConfigSession(IAiCoreApi api, AiConsentCoordinator consent) : IAsyncDisposable, IAiConfigCommands
{
    ...
    private ImmutableArray<AiToolFeature> _toolFeatures = [];

    public event EventHandler<AiConfigSnapshot>? StateChanged;
    public event EventHandler<AiConfigMutation>? MutationCompleted;

    public AiConfigSnapshot State => new(_channels, _credentials, _models, _presets, _toolFeatures);
    ...
    public async Task RefreshAsync(CancellationToken cancellationToken = default)
    {
        ...
        var toolFeatures = (await api.ListToolFeaturesAsync(cancellationToken)).ToImmutableArray();
        _channels = channels;
        _credentials = credentials;
        _models = models;
        _presets = presets;
        _toolFeatures = toolFeatures;
    ...
    public Task UpsertCredentialAsync(
        ...
        MutateAsync("credential", "upsert", id,
            () => api.UpsertCredentialAsync(id, channelId, label, endpoint, secret, true, null, null, cancellationToken));
    ...
    public Task CreateCredentialAsync(
        ...
        string? endpoint,
        string secret,
        IReadOnlyList<string>? initialModelIds,
        bool? thinkingEnabled,
        CancellationToken cancellationToken = default)
    {
        var id = Guid.NewGuid().ToString();
        return MutateAsync("credential", "create", id,
            () => api.UpsertCredentialAsync(id, channelId, label, endpoint, secret, true, initialModelIds, thinkingEnabled, cancellationToken));
    }
    ...
    public Task UpsertModelAsync(
        ...
        MutateAsync("model", "upsert", id,
            () => api.UpsertModelAsync(id, credentialId, modelId, displayName, source, true, contextWindowTokens, maxOutputTokens, thinkingEnabled, cancellationToken));
    ...
    public Task UpsertBindingAsync(string featureId, string modelProfileId, string? thinkingEffort, CancellationToken cancellationToken = default) =>
        MutateAsync("binding", "upsert", featureId,
            () => api.UpsertBindingAsync(featureId, modelProfileId, thinkingEffort, cancellationToken));

    public Task DeleteBindingAsync(string featureId, CancellationToken cancellationToken = default) =>
        MutateAsync("binding", "delete", featureId,
            () => api.DeleteBindingAsync(featureId, cancellationToken));
...
public sealed record AiConfigSnapshot(
    ...
    ImmutableArray<AiToolFeature> ToolFeatures);
