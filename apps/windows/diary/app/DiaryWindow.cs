...
public sealed class DiaryWindow : Window
{
    ...
    {
        _timeline.MoveNavigatorMonth(offset);
        BuildShell();
    }

    private void SelectTimelineDate(DateTime date)
    {
        _timeline.ToggleDate(date);
        EnsureSelectedEntryVisible();
        BuildShell();
    }
    ...
    {
        _timeline.NavigateToDate(month, date);
        BuildShell();
    }
    ...
    private (DateTime? Start, DateTime? End) TimelineDateWindow() => _timeline.DateWindow();
    ...
    private string NotebookName(string id) => _data.Notebooks.FirstOrDefault(notebook => string.Equals(notebook.Id, id, StringComparison.Ordinal))?.Name ?? _persistence.DefaultNotebookName;
    ...
    private async Task AddImageAttachmentAsync()
    {
        try
        {
            ...
            {
                _persistence.DeleteAttachment(attachment);
    ...
    private async Task DeleteEntryAsync(DiaryEntry entry)
    {
        ...
        if (SaveData(silent: true)) _persistence.DeleteAttachmentDirectory(entry.Id);
    ...
    private async Task ShowCreateNotebookDialogAsync()
    {
        ...
        _data.Notebooks.Add(new DiaryNotebook { Id = _persistence.CreateNotebookId(), Name = name.Text.Trim(), AccentColor = "#2F80FF" });
        SaveData(silent: true);
        BuildShell();
    }
    ...
    {
        var result = _persistence.SaveData(_data);
        if (result.Succeeded)
        {
            return true;
        }
    ...
    {
        var result = _persistence.SaveSettings(_settings);
        if (!result.Succeeded)
        {
            _ = ShowMessageAsync(
                "设置保存失败",
