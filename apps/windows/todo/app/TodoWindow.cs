...
public sealed class TodoWindow : Window
{
    ...
    {
        var generation = _presentation.NextTitleBarGeneration();
        DispatcherQueue.TryEnqueue(Microsoft.UI.Dispatching.DispatcherQueuePriority.Low, () =>
        {
            if (_presentation.IsCurrentTitleBarGeneration(generation))
            {
    ...
    {
        _presentation.InvalidateTitleBarGeneration();
        try
        {
            SetTitleBar(null);
    ...
    private void UpdateVirtualTitleBarRegions()
    {
        ...
        try
        {
            if (_presentation.HasModalSurface || !_brandArea.IsLoaded)
            {
    ...
    {
        _presentation.EnterModalSurface();
        ClearVirtualTitleBarRegions();
    }

    private void ExitModalSurface()
    {
        if (_presentation.ExitModalSurface())
        {
            QueueVirtualTitleBarRegionUpdate();
        }
    }
    ...
    private static TodoTaskDropPlacement SharedDropPlacement(TodoDropPlacement placement) => placement switch
    {
    ...
    {
        TodoTaskCommands.SetCompleted(_data, task.Id, completed: true, includeDescendants: true, now: DateTimeOffset.Now);
        _selectedTaskId = task.Id;
        SaveDataAndRefresh();
    }
    ...
    private bool IsDefaultList(string listId)
    {
        return string.Equals(listId, _store.DefaultListId, StringComparison.Ordinal);
    }
