...
public sealed partial class ConfigWindow : Window
{
    ...
    private async Task DeleteBindingAsync()
    {
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.bindings.delete", new { featureId = "ai.chat" });
    ...
    private async Task TestCredentialAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.credentials.test", new { credentialId = selected.Id });
    ...
    private async Task EditModelAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.models.upsert", new
            {
                id = selectedModel.Id,
    ...
    private async Task EditCredentialAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.credentials.upsert", new
            {
                id = selectedCredential.Id,
                channelId = selectedChannel.Id,
                label = label.Text,
                baseUrl = endpoint.Text,
                secret = string.IsNullOrWhiteSpace(secret.Password) ? null : secret.Password,
                enabled = true
            });
    ...
    private async Task DeleteChannelAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.channels.delete", new { id = selected.Id });
            await RefreshAllAsync();
        }
        catch (Exception exception) { ShowError(exception); }
    }
    ...
    private async Task RefreshAllAsync()
    {
        ...
        _presets.Clear();
        _presets.AddRange(await _client.InvokeAsync<List<AiPresetModel>>("ai.models.presets", new { }));
    ...
    private async Task AddChannelAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.channels.create", new
            {
                displayName = name.Text,
                defaultBaseUrl = endpoint.Text,
                enabled = true
            });
            await RefreshAllAsync();
        }
    ...
    private async Task DeleteCredentialAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.credentials.delete", new { id = selected.Id });
            await RefreshAllAsync();
        }
        catch (Exception exception) { ShowError(exception); }
    }
    ...
    private async Task AddModelAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.models.upsert", new
            {
                ...
                modelId = modelId.Text,
                displayName = string.IsNullOrWhiteSpace(displayName.Text) ? modelId.Text : displayName.Text,
                source = preset.SelectedItem is AiPresetModel ? "preset" : "custom",
                enabled = true
            });
            await RefreshAllAsync();
        }
    ...
    private async Task TestModelAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.models.test", new { modelProfileId = selected.Id });
            ShowMessage(L("AI_TestSuccess"), AiMessageSeverity.Success);
            await RefreshAllAsync();
        }
        catch (Exception exception) { ShowError(exception); }
    ...
    private async Task DeleteModelAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.models.delete", new { id = selected.Id });
            await RefreshAllAsync();
        }
        catch (Exception exception) { ShowError(exception); }
    }
    ...
    private async Task SaveBindingAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.bindings.upsert", new
            {
                featureId = "ai.chat",
