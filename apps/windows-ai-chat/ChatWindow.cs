...
public sealed partial class ChatWindow : Window
{
    ...
    public ChatWindow()
    {
        ...
        BuildContent();
        ConfigureWindow();
        _client.Notification += HandleNotification;
    ...
    private async Task RenameConversationAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.conversations.rename", new { id = selected.Id, title = title.Text });
    ...
    private async Task DeleteConversationAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.conversations.delete", new { id = selected.Id });
    ...
    private async Task RegenerateAsync()
    {
        ...
        try
        {
            var result = await _client.InvokeAsync<AiChatStarted>("ai.chat.regenerate", new
            {
    ...
    {
        var consent = await _client.InvokeAsync<AiConsent>("ai.consents.check", new { endpoint = credential.BaseUrl });
        if (consent.Granted)
        {
            return true;
        }
        ...
        {
            Text = string.Format(L("AI_CloudConsentDescription"), credential.BaseUrl),
            TextWrapping = TextWrapping.Wrap,
            MaxWidth = 520
        };
        ...
        await _client.InvokeAsync<AiConsent>("ai.consents.grant", new { endpoint = credential.BaseUrl });
        return true;
    }
    ...
    {
        _channels.Clear();
        _channels.AddRange(await _client.InvokeAsync<List<AiChannel>>("ai.channels.list", new { }));
        _credentials.Clear();
        _credentials.AddRange(await _client.InvokeAsync<List<AiCredential>>("ai.credentials.list", new { }));
        _models.Clear();
        _models.AddRange(await _client.InvokeAsync<List<AiModelProfile>>("ai.models.list", new { }));
        _conversations.Clear();
        _conversations.AddRange(await _client.InvokeAsync<List<AiConversationSummary>>("ai.conversations.list", new { }));
        FilterConversations();
    ...
    {
        var bindings = await _client.InvokeAsync<List<AiBinding>>("ai.bindings.list", new { });
    ...
    private async Task SelectConversationAsync()
    {
        ...
        try
        {
            var conversation = await _client.InvokeAsync<AiConversation>("ai.conversations.get", new { id = selected.Id });
    ...
    private async Task SendAsync()
    {
        ...
        try
        {
            var result = await _client.InvokeAsync<AiChatStarted>("ai.chat.send", new
            {
                conversationId = _currentConversationId,
                credentialId = credential.Id,
                modelProfileId = model.Id,
    ...
    private async Task StopAsync()
    {
        ...
        try
        {
            await _client.InvokeAsync<JsonElement>("ai.chat.cancel", new { invocationId = _activeInvocationId });
        }
        catch (Exception exception)
        {
            ShowError(exception);
    ...
    private void HandleNotification(object? sender, AiCoreNotificationEventArgs args)
    {
            ...
            switch (args.Method)
            {
                ...
                case "ai.chat.failed":
                    ...
                    if (finished is not null && finished.InvocationId == _activeInvocationId)
                    {
                        ...
                        {
                            var conversation = await _client.InvokeAsync<AiConversation>(
                                "ai.conversations.get",
                                new { id = _currentConversationId });
                            RenderMessages(conversation.Messages);
                        }
                    }
                    break;
