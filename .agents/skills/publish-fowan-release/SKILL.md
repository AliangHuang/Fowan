...
Do not generate, upload, or describe a portable ZIP. Fowan Windows releases are distributed only through the installer.

## Publish

Commit only source, docs, scripts, and metadata; `build/` and `publish/` are ignored. Create and push `v<version>` only when the user asks to publish. Upload all three delivery files to the GitHub Release, then read back the public `fowan-update.json` and verify its installer hash and URL. Never expose credentials or make a private repository public without explicit user confirmation.
